...
const upload = multer({
  ...
  fileFilter: (req, file, cb) => {
    if (constants.SUPPORTED_IMAGE_TYPES.includes(file.mimetype)) {
      ...
      const error = new Error(`Only image files are allowed (${constants.SUPPORTED_IMAGE_TYPES.join(', ')}).`);
...
const handleMulterError = (err, req, res, next) => {
  if (err instanceof multer.MulterError) {
    if (err.code === 'LIMIT_FILE_SIZE') {
      ...
      return res.status(400).json({ error: `File size too large. Maximum size is ${maxSizeMB}MB.` });
    }
